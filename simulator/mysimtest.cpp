...
string ticks="ticks_file.dat";  ofstream ticks_file(ticks);
...
float V_r_=(v+(b*omega/2))/k_r; //right tick
float V_l_=(v-(b*omega/2))/k_l; // left tick
while(scans < num_scans){

	if (size_intersection % 360 ==0){
         scans++;
		//* use ticks and k_l and k_b
  		pose.theta= theta(V_r_*(scans*period), V_l_*(scans*period),k_l, k_r, b);
  		pose.delta_x= x(V_r_*(scans*period), V_l_*(scans*period),k_l, k_r, b,pose.theta);
  		pose.delta_y= y(V_r_*(scans*period), V_l_*(scans*period),k_l, k_r, b,pose.theta);
		ticks_file << V_r_*(scans*period) << ' ' << V_l_*(scans*period) <<' ' << scans*period << endl  ;
