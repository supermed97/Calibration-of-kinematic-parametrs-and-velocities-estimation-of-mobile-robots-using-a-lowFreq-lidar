...
string line_vel;
ifstream vel_File;
vel_File.open("../micp_velocities.dat");
...
string line;
ifstream input_File;
input_File.open("../abs_ticks_file.dat");
...
float tottime;
  if(input_File.fail()) {
   ...
   // cerr<< tick_r << ' ' << tick_l << ' '<< endl; 
     ...
     else if (line_index<6 && line_index>=4){
  ...
  cerr<<" line_index: "<< line_index<<endl;
