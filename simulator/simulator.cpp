...
float polinomial(float theta, char dir){
 ...
 if(dir =='x'){
  ...
  return approx;}
	...
	float theta(float tick_r, float tick_l,float k_l, float k_r, float b){
	 float theta =   (k_r * tick_r) - (k_l *tick_l) ;
     theta = theta/b;
	// cout<< "I am IN FUNCTION THETA ! : " << k_r << " " << k_l << " " << tick_r << " "<< tick_l<< " " << b <<  " " << theta <<endl ;
	 return theta;
	  }

	float  x(float tick_r, float tick_l,float k_l, float k_r, float b,float theta){
	 float x =  ( (k_l *tick_l) + (k_r * tick_r) )/2;
	 x = x * polinomial(theta,'x');
	 return x;
	  }

	float  y( float tick_r,float tick_l,float k_l, float k_r, float b,float theta){
	 float y =  ( (k_l *tick_l) + (k_r * tick_r) )/2;
	 y = y * polinomial(theta,'y');
	 return y;
	  }
float  s( float tick_r,float tick_l,float k_l, float k_r ){
	 float s =  ( (k_l *tick_l) + (k_r * tick_r) )/2;
	 	 return s;
	  }
