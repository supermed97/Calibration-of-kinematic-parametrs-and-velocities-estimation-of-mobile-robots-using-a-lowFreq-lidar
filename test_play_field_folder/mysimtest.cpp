...
if (argc<5) {
    cerr << "usage: " << environ[0] << " RIGHT_WHEEL_VEL LEFT_WHEEL_VELOCITY FREQUENCY SIMULATION_PERIOD" << " NUM OF SCANS"<< endl ;
    return -1;
  }
              /** ODOMETRY PARAMETERS DEFINITION AND ASSIGNMENT **/
float period; float end_time = 0; float b = 5; float omega_r;  float omega_l; float sim_time;  int num_scans;
...
while(scans < num_scans){
	...
	if (size_intersection % 360 ==0){
		pose.theta= theta(omega_r/(scans+1)*period, omega_l/(scans+1)*period,2, 2, 5);
		pose.delta_x= x(omega_r/(scans+1)*period, omega_l/(scans+1)*period,2, 2, 5,pose.theta);
		pose.delta_y= y(omega_r/(scans+1)*period, omega_l/(scans+1)*period,2, 2, 5,pose.theta); 
		//sense_time=end_time;
